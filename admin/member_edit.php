...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (!empty($_POST["password"]) && !preg_match('/^[a-zA-Z0-9]{8,20}$/', $_POST["password"])) {
 ...
 if ($_POST["password"] !== $_POST["password_confirm"]) {
  ...
  if (empty($errorMessages["email"]) && $_POST["email"] != $_POST['before_edit_email']) {
    try {
      $dsn      = 'mysql:dbname=phpkadai;host=localhost;charset=utf8';
      $user     = 'kurihara';
      $password = 'uCmCLu2e8H';
...
// セッションからフォーム値を取得
$last_name    = isset($_POST['last_name']) ? $_POST['last_name'] : (isset($member['name_sei']) ? $member['name_sei'] : '');
$first_name   = isset($_POST['first_name']) ? $_POST['first_name'] : (isset($member['name_mei']) ? $member['name_mei'] : '');
...
<body>
  <div class="container">
    ...
    <form action="member_edit.php" method="post">
      ...
      <label>パスワード確認</label>
      <input type="password" name="password_confirm" value="<?= htmlspecialchars($password_confirm) ?>">
      <?php
      ...
      <input type="hidden" name="before_edit_email" value="<?= htmlspecialchars($member['email']) ?>">
      <?php
